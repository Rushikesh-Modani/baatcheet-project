...
function renderLoginForm(tab='login',role=null){
  ...
  if(tab==='login'){
    ...
    fc.innerHTML=`
      ...
      <div class="form-sub">Apna role chunein aur sign in karein</div>
      ...
      <div class="form-footer">Account nahi hai? <a onclick="renderLoginForm('register',loginRole)">Register karein</a></div>`;
  } else {
    if(role)loginRole=role;
    ...
    fc.innerHTML=`
      ...
      <div class="form-title">Account Banayein</div>
      <div class="form-sub">Pehle apna role chunein</div>
      <div class="role-selector">
      ...
      <div id="reg-fields">
        ${loginRole?renderRegFields(loginRole):'<div style="text-align:center;padding:20px;color:#aaaacc;font-size:13px;">Upar se role chunein</div>'}
      </div>
      <div class="form-footer" style="margin-top:14px">Pehle se account hai? <a onclick="renderLoginForm(\'login\',loginRole)">Log in karein</a></div>`;
  }
}
...
function updateLeftPanel(role){
  ...
  requestAnimationFrame(()=>{
    if(role==='therapist'){
      ...
      title.textContent='Welcome to Baatchete';
...
function doLogin(){
  ...
  if(!loginRole){showT('Role Chunein','Therapist ya Student Listener select karein','ic-red');return;}
  if(!email){errShake('l-id');showT('Email Zaroori Hai','Enter your email address','ic-red');return;}
  if(!isValidEmail(email)){errShake('l-id');showT('Email Sahi Nahi','Enter a valid email address','ic-red');return;}
  if(!pw){errShake('l-pw');showT('Password Zaroori Hai','Enter your password','ic-red');return;}
...
function doRegister(role){
  ...
  if(!name){errShake('r-name');showT('Naam Zaroori Hai','Enter your full name','ic-red');return;}
  if(!mob){errShake('r-mob');showT('Mobile Zaroori Hai','Enter your mobile number','ic-red');return;}
  if(!email){errShake('r-email');showT('Email Zaroori Hai','Enter your email address','ic-red');return;}
  if(!isValidEmail(email)){errShake('r-email');showT('Email Sahi Nahi','Enter a valid email address','ic-red');return;}
  if(role==='therapist'&&!rci){errShake('r-rci');showT('RCI Number Zaroori Hai','Enter your RCI number','ic-red');return;}
  if(role==='listener'&&!inst){errShake('r-inst');showT('Institute ID Zaroori Hai','Enter your Institute ID','ic-red');return;}
  if(role==='listener'&&!instname){errShake('r-instname');showT('Institute Name Zaroori Hai','Enter your Institute Name','ic-red');return;}
  if(!pw){errShake('r-pw');showT('Password Set Karein','Create a strong password','ic-red');return;}
  if(pw.length<6){errShake('r-pw');showT('Password Chota Hai','Kam se kam 6 characters ka password rakhein','ic-red');return;}
  if(pw!==pw2){errShake('r-pw2');showT('Password Match Nahi Kiya','Dono passwords same hone chahiye','ic-red');return;}
...
function showSuccessPanel(type,name){
  ...
  const titleTxt=type==='login'?'Login Ho Gaya!':'Account Ban Gaya!';
  const subTxt=type==='login'
    ?'Aapka Baatchete dashboard ready hai. Chalo shuru karte hain!'
    :`${name} ka account ${S.role==='therapist'?'Therapist':'Student Listener'} ke roop mein ban gaya. Dashboard explore karein!`;
  fc.innerHTML=`<div class="success-panel">
    ...
    <button class="${btnCls}" onclick="launchDashboard()">Dashboard Kholo</button>
  </div>`;
...
function doLogout(){
  ...
  activate('home');
  document.getElementById('nav-right').innerHTML=`<button class="btn-ghost-sm" onclick="showLogin()">Log In</button><button class="btn-orange-sm" onclick="showLogin()">Get Started</button>`;
  showT('Logged Out','Phir milenge!','ic-or');
}
...
function updateDashUI(){
  ...
  document.getElementById('d-sub').textContent='Aapka Baatchete Dashboard ready hai';
...
function toggleAvail(cb){
  ...
  if(S.isOnline){dot.className='a-dot on';txt.className='a-txt-on';txt.textContent='Online';showT('Ab Online Hain','Nayi requests aa sakti hain','ic-teal');}
  else{dot.className='a-dot off';txt.className='a-txt-off';txt.textContent='Offline';showT('Ab Offline Hain','Requests temporarily band hain','ic-or');}
}
...
function addDemoReq(){
  if(!S.isOnline){showT('Aap Offline Hain','Pehle online ho jao','ic-red');return;}
  const name=NMS[Math.floor(Math.random()*NMS.length)];
  const issue=ISS[Math.floor(Math.random()*ISS.length)];
  const rid=S.reqId++;
  S.requests.push({id:rid,name,issue,time:'Abhi',init:name[0]});
  renderReqs();updateBadge();
  document.getElementById('ov-req').textContent=S.requests.length;
  showT('Nayi Request!',`${name} aapse baat karna chahte hain`,'ic-or');
}

function renderReqs(){
  const el=document.getElementById('req-list');
  if(!S.requests.length){
    el.innerHTML=`<div class="card cp"><div class="empty-st"><div class="empty-ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg></div><div class="empty-t">Abhi koi request nahi</div><div class="empty-s">Availability "Online" rakho. Jab koi connect kare, yahan dikhega.</div></div></div>`;
    return;
  }
...
function acceptReq(id){
  ...
  showT('Session Accept Kiya',`Chat with ${r.name} started`,'ic-teal');
}
...
function declineReq(id){
  ...
  renderReqs();updateBadge();
  document.getElementById('ov-req').textContent=S.requests.length;
  showT('Request Decline Kiya',r?`${r.name} ki request decline kar di`:'','ic-red');
}
...
function renderSessions(){
  ...
  if(!tot){el.innerHTML=`<div class="empty-st"><div class="empty-ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/></svg></div><div class="empty-t">Koi session nahi</div><div class="empty-s">Requests accept karne ke baad sessions yahan dikhenge.</div></div>`;return;}
...
function saveProfile(){
  ...
  if(!name){showT('Naam Zaroori Hai','Enter your full name','ic-red');return;}
  S.profile={
    name,
  ...
  showT('Profile Save Ho Gaya',`Hello, ${name}!`,'ic-teal');
}
